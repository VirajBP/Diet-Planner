...
// GET /api/pexels/videos?query=exercise&per_page=10
router.get('/videos', async (req, res) => {
  const { query = 'exercise', per_page = 10 } = req.query;
  console.log('[Pexels Proxy] Incoming request:', req.method, req.originalUrl, '| Query:', query, '| Per page:', per_page);
  if (!PEXELS_API_KEY) {
    console.error('[Pexels Proxy] PEXELS_API_KEY not set in backend .env');
  ...
  try {
    ...
    console.log('[Pexels Proxy] Fetching from Pexels:', url);
    ...
    console.log('[Pexels Proxy] Data received:', data);
    res.json(data);
  } catch (error) {
